...
@cli.command()
@click.option('--input-dir', '-i', type=click.Path(exists=True), 
              required=True, help='数据目录')
def info(input_dir: str):
    """显示数据目录信息"""
    
    asyncio.run(_info_async(input_dir))


async def _info_async(input_dir: str):
    """异步信息显示函数"""
    ...
    try:
        ...
        console.print(f"📂 [bold]数据目录信息: {input_dir}[/bold]")
        
        # 扫描文件
        data_files = list(Path(input_dir).glob("*.json")) + list(Path(input_dir).glob("*.csv"))
        
        if not data_files:
            console.print("❌ [red]未找到数据文件[/red]")
            return
        
        console.print(f"📄 找到 {len(data_files)} 个数据文件")
        
        total_records = 0
        for file_path in data_files:
            try:
                data = await cleaner.clean_file(str(file_path))
                records = len(data)
                total_records += records
                console.print(f"  • {file_path.name}: {records} 条记录")
            except Exception as e:
                console.print(f"  • {file_path.name}: [red]读取失败 - {e}[/red]")
        
        console.print(f"\n📊 [bold]总计: {total_records} 条记录[/bold]")
